...
Application code **MUST** read the signed-in user from `Current.user`. The generated
`Authentication` concern exposes `authenticated?` and sets `Current.session`; the generated
`Current` model delegates `user` to that session. There is no `current_user` helper, so calling
one raises `NoMethodError`.

The generator covers sessions only. Everything else **MUST** be built or delegated to a gem:

| Capability | Generated by `bin/rails generate authentication` |
| ---------- | ------------------------------------------------ |
| Sign in, sign out, session tracking | Yes |
| Password reset over email | Yes |
| Rate-limited session creation | Yes |
| Sign-up and email confirmation | No |
| TOTP, WebAuthn, or passkeys | No |
| OAuth account linking | No |

Applications needing those flows **SHOULD** either add them deliberately — a registrations
controller with the same `normalizes` and `has_secure_password` rules, a signed confirmation
token with a short expiry, and a second factor stored per user — or use Devise[^25] with the
extensions below.
...
Long-running jobs **SHOULD** include `ActiveJob::Continuable` so that work survives a restart:
...
class DataExportJob < ApplicationJob
  ...
  def perform(export_id)
    # Code outside a step runs on every attempt, so a resumed job rebuilds
    # the state it needs. Completed steps are skipped.
    @export = Export.find(export_id)

    step :start_export do
      @export.start!
    end

    step :process_records do |step|
      @export.records.find_each(start: step.cursor) do |record|
...
Jobs **MUST NOT** carry iteration state between steps in instance variables:

```ruby
# BAD: @records is nil after a resume, and the loop saves no progress
step :fetch_records do
  @records = export.records.to_a
end

step :process_records do
  @records.each_with_index do |record, i|
    export.process_record(record)
    checkpoint! if i % 100 == 0
  end
end

# GOOD: reload durable state outside the steps, advance a serialised cursor
step :process_records do |step|
  export.records.find_each(start: step.cursor) do |record|
    export.process_record(record)
    step.advance! from: record.id
...
**Why**: A resumed job replays `perform` from the top and skips steps that already finished, so
`@records` is `nil` on the second attempt. Only the cursor and the completed-step list are
serialised with the job.

**Checkpoints are not progress**: `step.advance!` and `step.set!` record the cursor *and* create
a checkpoint; `checkpoint!` alone only offers an interruption point. A checkpoint interrupts the
job only when `queue_adapter.stopping?` is true, which the Solid Queue[^21] adapter reports once
a worker begins shutting down. Active Job also checkpoints before every step except the first of
each execution. Interrupted jobs are re-enqueued automatically, which is what prevents work loss
during Kamal's 30-second shutdown window.

## Caching: Solid Cache (Rails 8+)

Projects **SHOULD** use Solid Cache[^22] instead of Redis/Memcached for caching. New Rails 8
applications are configured for it already; older applications **MUST** run the installer, which
writes `config/cache.yml`, the cache schema, and the production cache store:
...
Cache-store options **MUST** be nested under `store_options`, and sizes and durations **MUST**
be evaluated with ERB:
...
Then load the cache schema with `bin/rails db:prepare`.

**Why nesting and ERB?** `SolidCache::Configuration` accepts only `store_options`, `database`,
`databases`, `connects_to`, `executor`, `encrypt`, `encryption_context_properties`, and
`size_estimate_samples`. Top-level cache options raise
`ArgumentError: unknown keywords: :max_size, :max_age` before the cache serves a single read.
YAML has no Ruby duration literals either, so an unevaluated `256.megabytes` reaches Solid Cache
as the string `"256.megabytes"`.

**Why a separate cache database?** A dedicated `database:` entry keeps cache writes out of the
primary connection pool and out of primary backups. It is not mandatory: with no `database`,
`databases`, or `connects_to` key, Solid Cache uses the `ActiveRecord::Base` pool, which makes
cache writes part of any enclosing transaction and rolls them back with it.
...
class Rack::Attack
  ...
  throttle("password_resets/email", limit: 3, period: 1.hour) do |req|
    next unless req.path == "/password" && req.post?

    user = req.params["user"]
    next unless user.is_a?(Hash)

    email = user["email"]
    next unless email.is_a?(String)
...
**Why the shape checks?** Discriminators run in middleware, before any controller or strong
parameters. An attacker controls the whole body, so `req.params["user"]` may be missing, a
string, or an array. `req.params["user"]["email"]` raises `NoMethodError` on an empty POST and
`TypeError` on `user[]=a`, turning a throttle into a 500. `dig` alone is not enough either:
`String` and `Array` do not respond to `dig` the way `Hash` does. Returning `nil` — via `next` —
means "do not throttle this request", which is the correct outcome for a request that carries no
email to throttle.

**Why hash the email?** The discriminator becomes part of the cache key. Hashing a canonicalised
address keeps plaintext addresses out of Redis while still bucketing `" Alex@Example.COM "` and
`"alex@example.com"` together.

Rate-limiting rules **MUST** be tested with malformed input:

```ruby
# spec/requests/password_reset_throttle_spec.rb
RSpec.describe "POST /password", type: :request do
  malformed = [
    "",                       # no body
    "user=",                  # empty scalar
    "user=scalar",            # string instead of hash
    "user[]=a&user[]=b",      # array instead of hash
    "user[name]=bob",         # no email key
    "user[email][]=a",        # array email
    "user[email]=%20%20"      # blank email
  ]

  malformed.each do |body|
    it "does not fail on #{body.inspect}" do
      post "/password", params: body,
  ...
  it "throttles the fourth reset for one address in an hour" do
    4.times { post "/password", params: {user: {email: "alex@example.com"}} }
...
applications that need registration, confirmation, lockable accounts, and OAuth providers
without writing those flows. Devise core has **no** two-factor module: TOTP requires the
separate devise-two-factor[^38] extension, and passkeys require a WebAuthn library. Choose
Devise for the account-management breadth, not for MFA.
...
class ApplicationController < ActionController::Base
  ...
  # Pundit calls current_user by default; generated Rails 8 authentication
  # supplies Current.user instead.
  def pundit_user
    Current.user
  end
end
...
Applications that switch the signed-in user mid-request — impersonation, sign-in after sign-up —
**MUST** call `pundit_reset!` after changing the session, because Pundit memoises `pundit_user`
for the request.
...
tested by running them more than once, not by asserting that a duplicate was never enqueued.

### Idempotent Jobs

Every queue delivers a job at least once: a worker that dies after sending an email but before
acknowledging the job will run it again. Jobs with external side effects **MUST** therefore
guard the side effect in the database, under a unique index:

```ruby
# db/migrate/20260908000001_create_order_notifications.rb
class CreateOrderNotifications < ActiveRecord::Migration[8.1]
  def change
    ...
    add_index :order_notifications, [:order_id, :kind], unique: true
  end
end
```
...
class OrderNotificationJob < ApplicationJob
  ...
  def perform(order_id)
    ...
    notification = OrderNotification.create_or_find_by!(order_id: order.id, kind: KIND)

    claimed = OrderNotification
      .where(id: notification.id, delivered_at: nil)
...
RSpec.describe OrderNotificationJob do
  ...
  it "delivers one email however many times it runs" do
    expect {
      3.times { described_class.perform_now(order.id) }
    }.to change { ActionMailer::Base.deliveries.count }.by(1)
  end

  it "delivers one email under concurrent execution" do
    threads = 5.times.map do
      Thread.new do
...
**Why claim before delivering?** The conditional `update_all` is a single atomic statement, so
exactly one execution sees `claimed == 1` even when several run at once. `create_or_find_by!`
inserts inside a savepoint in Rails 8.1, so the losing insert does not abort a surrounding
transaction. Claiming first makes delivery at-most-once: a crash between the claim and the send
drops that email. Where an email matters more than a rare duplicate, send first and record
afterwards, or pass an idempotency key to the mail provider and let it deduplicate.

Jobs **MUST NOT** mix the two job APIs:

```ruby
# BAD: raises ArgumentError, "Sidekiq::Job cannot be included in an ActiveJob"
class OrderNotificationJob < ApplicationJob
...
Projects that run Sidekiq[^14] directly **MUST** write a native job class, and **MUST** register
the sidekiq-unique-jobs[^15] middleware themselves — it has not been automatic since v7, and the
gem does not support Active Job:

```ruby
# app/sidekiq/order_notification_job.rb
class OrderNotificationJob
  ...
  def perform(order_id)
    # Same database claim as above: locks stop duplicate enqueues,
    # not duplicate executions.
  end
end
```
...
**Why not treat a lock as idempotence?** `lock: :until_executed` suppresses a *duplicate enqueue*
while the first job is queued. It does nothing about a retry after the job already sent the
email, which is the case that produces the second message.

### Idempotent Endpoints

Endpoints that create resources **SHOULD** accept an `Idempotency-Key` header and **MUST**
reserve it atomically, scoped to the actor and the endpoint:

```ruby
# db/migrate/20260908000002_create_idempotent_requests.rb
class CreateIdempotentRequests < ActiveRecord::Migration[8.1]
  def change
    create_table :idempotent_requests do |t|
      ...
      t.integer :status
      t.json :response_body  # jsonb on PostgreSQL
      t.timestamps
    end

    add_index :idempotent_requests,
      [:user_id, :endpoint, :idempotency_key],
      unique: true,
      name: "index_idempotent_requests_on_actor_and_key"
  end
end
```
...
class Api::V1::OrdersController < Api::V1::BaseController
  ...
  def create
    ...
    if key.nil?
      return render json: {error: "Idempotency-Key header is required"},
        status: :bad_request
    end

    record = completed_request(key) || create_order(key)

    if record
      render json: record.response_body, status: record.status
    else
      render json: {error: "A request with this Idempotency-Key is still in flight"},
        status: :conflict
    end
  end
  ...
  def completed_request(key)
    IdempotentRequest.completed.find_by(
      user: Current.user, endpoint: ENDPOINT, idempotency_key: key
    )
  end

  # Reserves the key and does the work in one transaction. Returns nil when
  # another request holds the key and has not finished yet.
  def create_order(key)
    IdempotentRequest.transaction do
      record = IdempotentRequest.create!(
        user: Current.user, endpoint: ENDPOINT, idempotency_key: key
      )
      order = Orders::CreateService.call(user: Current.user, params: order_params)
      record.update!(
        status: 201,
        response_body: order.as_json(only: [:id, :quantity, :status])
      )
      record
    end
  rescue ActiveRecord::RecordNotUnique
    completed_request(key)
  end
end
```

```ruby
# app/models/idempotent_request.rb
...
RSpec.describe "POST /api/v1/orders", type: :request do
  ...
  it "replays the stored response for a repeated key" do
    post api_v1_orders_path, params: {order: {quantity: 2}}, headers: headers
  ...
  it "scopes keys to the actor" do
    create(:idempotent_request, user: create(:user), endpoint: "POST /api/v1/orders",
      idempotency_key: "key-123", status: 201, response_body: {"id" => 0})

    post api_v1_orders_path, params: {order: {quantity: 2}}, headers: headers

    expect(response).to have_http_status(:created)
    expect(JSON.parse(response.body)["id"]).not_to eq(0)
  end

  it "returns 409 while another request holds the key" do
    IdempotentRequest.create!(
      user: user, endpoint: "POST /api/v1/orders", idempotency_key: "key-123"
    )
  ...
  it "leaves no reservation behind when the operation fails" do
    failure = Orders::CreateService::Error.new("payment declined", order: Order.new)
    allow(Orders::CreateService).to receive(:call).and_raise(failure)

    expect {
      post api_v1_orders_path, params: {order: {quantity: 2}}, headers: headers
    }.to raise_error(Orders::CreateService::Error)

    expect(IdempotentRequest.count).to eq(0)
  end

  it "creates one order for simultaneous requests with the same key" do
    statuses = Concurrent::Array.new

    threads = 4.times.map do
      Thread.new do
        # Rack::Test sessions are not thread-safe; call the app directly.
        ActiveRecord::Base.connection_pool.with_connection do
          env = Rack::MockRequest.env_for(
            api_v1_orders_path,
            method: "POST",
            params: {order: {quantity: 2}},
            "HTTP_IDEMPOTENCY_KEY" => "key-123",
            **credentials_for(user)  # whatever your API authenticates with
          )
          statuses << Rails.application.call(env).first
        end
      end
    end
    threads.each(&:join)
    ...
    expect(Order.count).to eq(1)
    expect(statuses.uniq - [201, 409]).to be_empty
  end
end
```

**Why a transaction and a unique index?** The published anti-pattern is a `find_by` followed by a
`create!`:

```ruby
# BAD: two requests both find nothing, both create an order
...
IdempotentRequest.create!(key: idempotency_key, response: order)
```

Nothing prevents two requests from passing the `find_by` check together, the service is called
without the `user:` it requires, an Active Record object is stored where a serialised body
belongs, and a key from one customer collides with the same key from another. Reserving the row
inside the transaction that does the work fixes all four: the unique index rejects the second
reservation, a failure rolls the reservation back so the client can retry, and the persisted
status and body make the replay identical to the original response.
...
[^38]: **devise-two-factor** - TOTP second factor for Devise. Devise core provides no MFA module. [https://github.com/devise-two-factor/devise-two-factor](https://github.com/devise-two-factor/devise-two-factor)
